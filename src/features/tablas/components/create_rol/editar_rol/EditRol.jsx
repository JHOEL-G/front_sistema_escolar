...
import { X, Sparkles, Save, CheckCircle2, Loader2 } from 'lucide-react';
import serviceApiNet from '../../../../../lib/api/serviceApiNet';
import { InputConError } from '../../diseño_formulario/DiseñoFormulario';

const EditRol = ({ onClose, onSuccess, rolId }) => {
    ...
    useEffect(() => {
        const cargarRol = async () => {
            try {
                setLoadingRol(true);
                const res = await serviceApiNet.Roles.getById(rolId);
                const rol = res.data?.data || res.data;
    ...
    const manejarEnvios = async () => {
        ...
        try {
            ...
            await serviceApiNet.Roles.update(rolId, rolData);
        ...
        return (
            <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-end z-50">
                <div className="w-[450px] bg-white m-4 shadow-2xl flex flex-col items-center justify-center rounded-3xl">
                    ...
                    <p className="text-slate-500 font-medium mt-4">Cargando rol...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="flex justify-between items-start p-6 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-1">
                            {step === 5 ? '¡Cambios guardados!' : 'Modifica la información del rol'}
                        </p>
                    </div>
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-3 duration-400 space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">Modifica los datos del rol</p>
                                </div>
                            </div>
