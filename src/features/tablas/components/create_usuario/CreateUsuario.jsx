...
import { EyeOff, Eye, X, Upload, ChevronRight, ChevronLeft, Save, AlertCircle, CheckCircle2, RefreshCw, Sparkles } from 'lucide-react';
...
const CreateUsuario = ({ onClose, onSuccess }) => {
    ...
    const manejarEnvios = async () => {
        ...
        try {
            ...
            const nivelPermisoId = mapeoNiveles.find(n => n.nombre === formData.nivelPermiso)?.id
                || Number(formData.nivelPermiso)
                || 4;
            ...
            const response = await serviceApiNet.Keycloak.createUsuarioKeycloak(formDataToSend);
            setStep(5);
            if (onSuccess) onSuccess();
            setTimeout(() => { if (onClose) onClose(); }, 5000);
                ...
                <div className="flex justify-between items-start p-6 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-1">
                            {step === 5 ? '¡Proceso completado!' : `Paso ${step} de 4`}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {step <= 4 && (
                    <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-b from-slate-50 to-white">
                        <div className="flex items-center justify-between relative">
                            ...
                            <div className={`flex-1 h-0.5 mx-3 mb-6 transition-all duration-500 ${step > 1 ? 'bg-gradient-to-r from-indigo-500 to-purple-500' : 'bg-slate-200'}`}></div>
                            <StepItem label="Acceso" active={step === 2} completed={step > 2} number={2} />
                            <div className={`flex-1 h-0.5 mx-3 mb-6 transition-all duration-500 ${step > 2 ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-slate-200'}`}></div>
                            <StepItem label="Organización" active={step === 3} completed={step > 3} number={3} />
                            <div className={`flex-1 h-0.5 mx-3 mb-6 transition-all duration-500 ${step > 3 ? 'bg-gradient-to-r from-pink-500 to-rose-500' : 'bg-slate-200'}`}></div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-full px-4 py-3 pr-24 border-2 rounded-xl outline-none transition-all duration-200 ${errors.contraseña
                                    ...
                                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                        ...
                                        >
                                            <RefreshCw className="text-slate-400 hover:text-indigo-500" size={16} />
                                        </button>
                                        <button
                                            type="button"
                                        ...
                                        >
                                            {showPassword ? <Eye className="text-slate-400 hover:text-indigo-500" size={16} /> : <EyeOff className="text-slate-400 hover:text-indigo-500" size={16} />}
                                        </button>
                                    </div>
                                </div>
                                {errors.contraseña && (
                                ...
                                {formData.contraseña && !errors.contraseña && (
                                    <div className="flex items-center gap-2 text-emerald-600 text-xs font-medium animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    {step === 3 && (
                        <div className="animate-in fade-in slide-in-from-bottom-3 duration-400 space-y-5">
                            ...
                            <SelectConError
                                ...
                                onChange={manejarCambio}
                                options={unidades_organizacionales}
                                errors={errors}
                                required
                            />
                    ...
                    {step === 4 && (
                        <div className="animate-in fade-in slide-in-from-bottom-3 duration-400 space-y-5">
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex justify-between items-center py-2 border-b border-indigo-100/50">
                                        ...
                                        <span className="text-slate-800 font-semibold">
                                            {roles.find(r => r.value === formData.rolId.toString())?.label || '-'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center py-2">
                                        <span className="text-slate-600 font-medium">Permiso:</span>
                                        <span className="text-slate-800 font-semibold">
                                            {mapeoNiveles.find(n => n.id === Number(formData.nivelPermiso))?.nombre || formData.nivelPermiso}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 mt-6">
                                ...
                                <label
                                    htmlFor="foto-upload"
                                    className="border-2 border-dashed border-slate-300 rounded-2xl p-6 flex flex-col items-center justify-center bg-slate-50/50 hover:bg-slate-100/50 hover:border-indigo-300 cursor-pointer transition-all duration-200 group block"
                                >
                                    {imagePreview ? (
                                        <div className="relative">
                    ...
                    {step === 5 && (
                        <div className="animate-in fade-in zoom-in duration-500 flex flex-col items-center justify-center py-16">
                            ...
                            <h3 className="text-3xl font-bold text-slate-800 mt-8 mb-3">¡Usuario Creado!</h3>
                    ...
                    <div className="p-6 border-t border-slate-100 bg-white flex justify-between items-center">
                        {step > 1 ? (
                            <button
                                ...
                                className="text-slate-600 font-semibold px-6 py-2.5 hover:bg-slate-100 rounded-xl transition-colors duration-200 flex items-center gap-2"
                                disabled={loading}
                            >
                            ...
                            <button
                                onClick={onClose}
                                className="text-slate-600 font-semibold px-6 py-2.5 hover:bg-slate-100 rounded-xl transition-colors duration-200"
                            >
                                Cancelar
                            </button>
                        )}
                        ...
                        {step < 4 ? (
                            <button
                                ...
                                className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-3 rounded-xl font-semibold flex items-center gap-2 hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg shadow-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-10 py-3 rounded-xl font-semibold flex items-center gap-2 hover:from-emerald-700 hover:to-teal-700 transition-all duration-200 shadow-lg shadow-emerald-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {loading ? (
                                    <>
