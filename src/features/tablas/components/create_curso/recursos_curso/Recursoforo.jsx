...
import { X, MessageSquare, BarChart3, Globe, Lock, Pencil, Check, Sparkles, ChevronDown, Upload, Paperclip } from "lucide-react";
import { getFileConfig } from "./diseño_evaluacion/items/Item";

const RecursoForo = ({ recursoData, onSave, onCancel }) => {
    const cfg = recursoData?.configuracion || {};
    const archivosExistentes = cfg.Archivos || cfg.archivos ||
        cfg.foroArchivos || cfg.ForoArchivos ||
    ...
    const [formData, setFormData] = useState({
        ...
        privacidad: recursoData?.privacidad || cfg.Privacidad || cfg.privacidad || "Público"
    });
    ...
    const handleEliminarNuevo = (index) => {
        setArchivosNuevos(prev => prev.filter((_, i) => i !== index));
    };

    const handleEliminarExistente = (index) => {
        setArchivosExistentesActivos(prev => prev.filter((_, i) => i !== index));
    };
            ...
            <div className="relative p-10 pb-8 border-b border-slate-100">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-sky-400 shadow-lg shadow-sky-200 rounded-2xl flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div className="flex items-center gap-2 group/titulo">
                                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">
                                            {formData.tituloSesion
                                                ...
                                                : <><span className="text-slate-300">Configurando </span><span className="text-sky-400">Foro</span></>}
                                        </h2>
            ...
            <div className="px-10 py-8 space-y-10">
                <section>
                    <div className="mb-5">
                        ...
                        <p className="text-xs text-slate-400">Datos generales del foro</p>
                    </div>
                    <div className="space-y-4">
                        <div>
                            ...
                            <textarea value={formData.descripcionSesion} onChange={(e) => handleInputChange("descripcionSesion", e.target.value)}
                                placeholder="Describe brevemente el propósito de este foro..."
                        ...
                        <div>
                            ...
                            <textarea value={formData.instrucciones}
                                ...
                                placeholder="Escribe las instrucciones para los participantes..."
                ...
                <section>
                    <div className="mb-5">
                        ...
                        <p className="text-xs text-slate-400">Parámetros de comportamiento del foro</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <button type="button" onClick={() => handleInputChange("privacidad", formData.privacidad === "Público" ? "Privado" : "Público")}
                            ...
                            <div>
                                <p className={`font-bold text-sm transition-colors ${formData.privacidad === "Privado" ? 'text-indigo-700' : 'text-slate-800'}`}>{formData.privacidad === "Privado" ? "Foro Privado" : "Foro Público"}</p>
                                <p className="text-[10px] text-slate-400 font-medium mt-0.5 uppercase tracking-tighter">{formData.privacidad === "Privado" ? "Solo participantes inscritos" : "Visible para todos"}</p>
                            </div>
